...
const existingContractAddr = "0x20699EC7f524B5914D30a9dd532A86B0494003aC";

async function main() {
  const game = await hre.ethers.getContractAt("Game", existingContractAddr);

  const signer0 = await ethers.provider.getSigner(0);
  const nonce = await signer0.getTransactionCount();
  for(let i = 0; i < friends.length; i++) {
    await game.levelUp(friends[i], {
      nonce: nonce + i
    });
  }

  console.log("Level ups are complete!");
}
